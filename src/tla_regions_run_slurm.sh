...
then
    ...
    steps=$(sbatch --array=1-$ncases --parsable --export=STUDY=$1,GRAPH=$2,REDO=$3 src/tla_regions_run_sbatch.sh)
	...
	# create a temporary parameter file pointing to sub-sample list
        ...
        mkdir -p log
        # >> use {$2, $3 = --graph, --redo} for optional graphing or redoing calculations 	
        steps=$(sbatch --array=1-$ncases --parsable --export=STUDY=$studtbl,GRAPH=$2,REDO=$3 src/tla_regions_run_sbatch.sh)
