...
read name raw_path raw_samples_table raw_classes_table raw_reg_classes_table data_path rest < <(sed "1d" "$1")
samples_files=$raw_path/$raw_samples_table
ncases=$(($(numlines $samples_files)))

echo "TLA_reg_sum: Processing ($ncases) samples in study <$1>" 

# run the setup summary
mkdir -p log
sums=$(sbatch --parsable --export=STUDY=$1 src/tla_regions_sum_sbatch.sh)
