'''
    ...
    ########################
        ...
        different samples, as tiles of the initial large sample. 
        
        This allows for using TLA in samples that are very large.
...
def pick_rois(cell_data, classes, sid, imshape, split_rois, binsiz, 
       ...
       """
       from tla_functions import landscapeScatter, plotEdges, toint
       ...
       def do_axes(ax):
           from tla_functions import setTicklabelSpacing
...
def main(args):
    ...
    if debug:
        # running from the IDE, path of directory containing this script
        main_pth = os.path.dirname(os.getcwd())
        argsfile = os.path.join(main_pth, 'test_set_reg.csv')

    else:
        # running from the CLI, path to working directory (above /scr)
        main_pth = os.getcwd()
    ...
    for c in  range(len(study.samples)):
        ...
        # plot scatter plot and then scatter plot
        slide.plot_landscape_scatter(slide.split_roi_mask())
...
if __name__ == "__main__":
    ...
    # Create the parser
    my_parser = ArgumentParser(prog="tla_slide_splits",
                               description="# selects ROI masks and " +
                               "draws scatter plots for each slide. " +
                               "Module 0 for Tumor Landscape Analysis #",
                               allow_abbrev=False)
